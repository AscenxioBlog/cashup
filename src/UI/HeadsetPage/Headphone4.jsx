...
// import CartNotification from '../../ReusableComponent/ButtonFolder/CartNotification';
          ...
          <div key={item.id} className="min-h-[400px] md:h-[520px] bg-slate-200 p-2 relative">
            ...
            <div className="absolute top-0 bottom-0 right-0 left-0">
              <div className="btnparent h-[100%] w-full relative">
                <CustomBtn
                 onClick={ () =>{
                  toast.success(`${item.name} successfully addedd to cart`);
                   addToCart(item);
                 }}
                  label="ADD TO CART"
...
{/* {currentItems.map((item) => (
        <div key={item.id} className="bg-slate-200 p-4 shadow-md rounded-lg">
          <img src={item.image} alt={item.name} className="h-[200px] w-full object-cover mb-3" />
          ...
          <div className="mt-4 flex gap-3">
            <CustomBtn
              label="ADD TO CART"
              className="bg-btncolor hover:bg-btnactive text-white rounded-lg px-4 py-2"
